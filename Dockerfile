...
LABEL maintainer="Dave Conroy (dave at tiredofit dot ca)"
    ...
    RUN export CPU=`cat /proc/cpuinfo | grep -c processor` \
        ...
        && mkdir -p /usr/src \
        && mkdir -p /etc/mysql \
        && wget -O /usr/src/mdb.tar.gz http://mariadb.mirror.iweb.com/mariadb-${MARIADB_VER}/source/mariadb-${MARIADB_VER}.tar.gz \
        && cd /usr/src && tar -xf mdb.tar.gz && rm mdb.tar.gz \
        # Build maridb
        && mkdir -p /tmp/_ \
